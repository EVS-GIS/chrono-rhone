    ...
    <form @submit.prevent="CreateTheme">
      <div class="modal-card">
        <header class="modal-card-head">
          <p class="modal-card-title">{{$t('themes.title-create')}}</p>
        </header>
        <section class="modal-card-body">
          ...
          <ValidationProvider rules="required" name="thematic" v-slot="{ errors, valid }">
            <b-field :label="$t('fields.thematic')" :type="{ 'is-danger': errors[0], 'is-success': valid }" :message="errors">
              <b-select v-model="form.thematic_id">
...
export default {
  ...
  methods: {
    ...
    async CreateTheme() {
      try{
        const response = await axios.post('admin/themes', this.form)
        if(response.status === 200){
          this.$buefy.snackbar.open({
            message: this.$t('themes.created'),
            position: 'is-top',
            type: 'is-success'
          })
          this.$parent.close()
