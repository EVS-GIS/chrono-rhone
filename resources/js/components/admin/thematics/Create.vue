<template>
  <ValidationObserver ref="observer" v-slot="{ invalid }">
    <form @submit.prevent="CreateThematic">
      <div class="modal-card">
        <header class="modal-card-head">
          <p class="modal-card-title">{{$t('thematics.title-create')}}</p>
        </header>
...
export default {
  ...
  methods: {
    ...
    async CreateThematic() {
      try{
        const createform = Object.assign({},this.form)
        createform.themes = this.form.themes.map((theme) => {return theme.id})
        const response = await axios.post('admin/thematics', createform)
        if(response.status === 200){
          this.$buefy.snackbar.open({
            message: this.$t('thematics.created'),
            position: 'is-top',
            type: 'is-success'
          })
          this.$parent.close()
