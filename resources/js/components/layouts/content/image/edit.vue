    ...
    <form @submit.prevent="EditImage">
      <div class="modal-card">
        <header class="modal-card-head">
          <p class="modal-card-title">{{$t('images.title-update')}}</p>
        </header>
        <section class="modal-card-body">
          ...
          <ValidationProvider rules="required" name="legend" v-slot="{ errors, valid }">
            <b-field :label="$t('fields.legend')" :type="{ 'is-danger': errors[0], 'is-success': valid }" :message="errors">
              <b-input type="textarea" v-model="image['legend_'+lang]"></b-input>
            </b-field>
          </ValidationProvider>
          <ValidationProvider rules="required" name="copyright" v-slot="{ errors, valid }">
            <b-field :label="$t('fields.copyright')" :type="{ 'is-danger': errors[0], 'is-success': valid }" :message="errors">
              <b-input type="text" v-model="image.copyright"></b-input>
            </b-field>
          </ValidationProvider>
...
export default {
  ...
  data() {
    return {
      linkedEvents:this.image.events,
      events: [],
      filteredEvents: [],
    }
  },
  ...
  computed:{
    ...
    formdata() {
      ...
      formData.events = this.linkedEvents.map(event=>event.id)
      return formData
    }
  },
  methods: {
    ...
    async EditImage() {
      try{
        ...
        const response = await axios.put('image/'+this.image.id, this.formdata)
