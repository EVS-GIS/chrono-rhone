      ...
      <div class="modal-card">
        <header class="modal-card-head has-background-link">
          ...
          <div class="column is-10">
            <p class="modal-card-title has-text-white">{{$t('events.title-create')}}</p>
          </div>
          ...
          <b-field :label="$t('single.previous')">
            <b-table
              ...
              <template slot="footer">
                <th>
                  <b-field>
                    <b-autocomplete v-model="fromRelatedEvent" :data="fromFilteredEvents" field="title_fr" :placeholder="$t('fields.event')" clearable expanded @select="option => (fromSelectedEvent = option)">
          ...
          <b-field :label="$t('single.next')">
            <b-table
              ...
              <template slot="footer">
                <th>
                  <b-field>
                    <b-autocomplete v-model="toRelatedEvent" :data="toFilteredEvents" field="title_fr" :placeholder="$t('fields.event')" clearable expanded @select="option => (toSelectedEvent = option)">
                      <template slot="empty">No results found</template>
                    </b-autocomplete>
                  </b-field>
                </th>
...
export default {
  ...
  computed:{
    ...mapGetters(['isAdmin','isEditor','user']),
    lang(){
    ...
    creator(){
      const creator = this.users.find(user=> user.id === this.event.author_id)
  ...
  methods: {
    ...
    async CreateEvent() {
      try {
        if(!this.event.author_id){
          this.event.author_id = this.user.id
        }
        this.event.creator = this.creator
        const response = await axios.post('event', this.event)
        if(response.status === 200){
          this.$buefy.snackbar.open({
            message: this.$t('events.created'),
            position: 'is-top',
            type: 'is-success'
          })
          this.$parent.close()
