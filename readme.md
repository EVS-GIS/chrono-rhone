...
# Deploy this app using docker-compose

In this example, database is stored in a persistent docker volume. 

- Copy the example .env file and make the required configuration changes in the .env file (database, url, email, mapbox token, etc...)
...
- If the database is a new empty instance, migrate and seed it
        ...
        docker exec -it --rm -w /var/www/chrono-rhone/ chrono-rhone_app_1 /usr/local/bin/php /var/www/chrono-rhone/artisan migrate --force
        docker exec -it --rm -w /var/www/chrono-rhone/ chrono-rhone_app_1 /usr/local/bin/php /var/www/chrono-rhone/artisan db:seed --force
